...
def _resolve_template_from_text(text: str) -> Optional[str]:
    ...
    for mode_key, tpl in TEMPLATES.items():
        title = _norm_q(tpl["title"])
        if title in q or mode_key in q:
            return mode_key
    if "confidence" in q:
        return "build_confidence"
    if "difficult" in q or "behav" in q or "push back" in q:
...
def _is_template_invocation_text(mode: str, query: str) -> bool:
    q = _norm_q(query)
    tpl = TEMPLATES.get(mode) or {}
    title = _norm_q(tpl.get("title"))
    mode_key = _norm_q(mode)
    return title in q or mode_key in q
